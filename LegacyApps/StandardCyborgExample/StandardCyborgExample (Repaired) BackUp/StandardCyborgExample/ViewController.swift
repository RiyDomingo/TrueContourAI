...
class ViewController: UIViewController {
    ...
    private weak var activeScanningVC: ScanningViewController!
    ...
    @IBAction private func startScanning(_ sender: UIButton) {
        ...
        self.activeScanningVC = scanningVC
        present(scanningVC, animated: true) {
            // Add a Finish button overlay for manual completion
            let finishButton = UIButton(type: .system)
            if #available(iOS 15.0, *) {
                var config = UIButton.Configuration.filled()
                config.title = "Finish"
                config.baseForegroundColor = .white
                config.baseBackgroundColor = UIColor(red: 0.14, green: 0.54, blue: 1.0, alpha: 0.9)
            ...
            finishButton.addTarget(self, action: #selector(self.finishScanNowTapped), for: .touchUpInside)
            finishButton.translatesAutoresizingMaskIntoConstraints = false
            scanningVC.view.addSubview(finishButton)
            // Pin to top-right with safe area insets
    ...
    @objc private func finishScanNowTapped() {
        activeScanningVC?.finishScanNow()
    }
