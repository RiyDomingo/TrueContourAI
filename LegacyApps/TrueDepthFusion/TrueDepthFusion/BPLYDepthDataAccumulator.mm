...
#define BPLY_HAS_SSZIPARCHIVE 1
#else
#define BPLY_HAS_SSZIPARCHIVE 0
#endif
...
#if BPLY_HAS_SSZIPARCHIVE
    NSString *zipPath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"DepthFrames.zip"];
    BOOL created = [SSZipArchive createZipFileAtPath:zipPath withContentsOfDirectory:_containerDirectory];
    if (!created) {
        NSLog(@"Failed to create zip at %@ using SSZipArchive; returning directory URL instead.", zipPath);
        return [NSURL fileURLWithPath:_containerDirectory isDirectory:YES];
    }
    return [NSURL fileURLWithPath:zipPath];
#else
    NSLog(@"SSZipArchive not available; returning directory URL instead of a zip archive.");
    return [NSURL fileURLWithPath:_containerDirectory isDirectory:YES];
#endif
}
