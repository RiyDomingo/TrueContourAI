...
class ScanningViewController: UIViewController, CameraManagerDelegate, SCReconstructionManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // Use a context-derived scale instead of deprecated UIScreen.main
        ...
        if let window = view.window ?? UIApplication.shared.connectedScenes.compactMap({ ($0 as? UIWindowScene)?.keyWindow }).first {
            contextScale = window.windowScene?.screen.scale ?? traitCollection.displayScale
        } else {
            contextScale = traitCollection.displayScale
        }
        _metalLayer.contentsScale = contextScale
