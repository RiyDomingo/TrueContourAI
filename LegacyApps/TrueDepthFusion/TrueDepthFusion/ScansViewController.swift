...
class ScansViewController: UITableViewController {
    ...
    private func _updateNoScansLabel() {
        ...
        if _noScansLabel.isHidden == false {
            let bounds: CGRect
            if view.window != nil {
                // Prefer the tableView's bounds to position within the visible area when in a window
                bounds = tableView.bounds
            } else {
                // Fallback before the view is in a window
                bounds = tableView.bounds
            }
            _noScansLabel.frame = bounds.offsetBy(dx: 0, dy: 0.5 * tableView.rowHeight - 88 - 0.5 * _noScansLabel.font.lineHeight)
        }
    }
