...
private final class HomeScanDetailsWorkflow {
    private let scanService: any ScanItemListing & ScanSummaryReading

    init(scanService: any ScanItemListing & ScanSummaryReading) {
...
private final class HomeScanLibraryEditWorkflow {
    private let scanService: any ScanFolderEditing
    private let previewSessionState: PreviewSessionState

    init(scanService: any ScanFolderEditing, previewSessionState: PreviewSessionState) {
        self.scanService = scanService
    ...
    func presentRename(
        ...
        alertVC.addAction(UIAlertAction(title: L("common.rename"), style: .default, handler: { [weak self] _ in
            ...
            case .success(let newURL):
                if self.previewSessionState.currentPreviewedFolderURL == item.folderURL {
                    self.previewSessionState.currentPreviewedFolderURL = newURL
                }
                onScansChanged()
            case .nameExists:
                presenter.present(
                    makeAlert(L("scan.flow.rename.nameExists.title"), L("scan.flow.rename.nameExists.message")),
                    animated: true
                )
            case .invalidName:
                presenter.present(
                    makeAlert(L("scan.flow.rename.invalid.title"), L("scan.flow.rename.invalid.message")),
                    animated: true
                )
            case .failure(let error):
                presenter.present(
                    makeAlert(L("scan.flow.rename.failed.title"), error.localizedDescription),
                    animated: true
                )
            }
        }))
    ...
    func presentDelete(
        ...
        alertVC.addAction(UIAlertAction(title: L("common.delete"), style: .destructive, handler: { [weak self] _ in
            ...
            case .success:
                if self.previewSessionState.currentPreviewedFolderURL == item.folderURL {
                    self.previewSessionState.currentPreviewedFolderURL = nil
                }
                onScansChanged()
            case .failure(let error):
                presenter.present(
                    makeAlert(L("scan.flow.delete.failed.title"), error.localizedDescription),
                    animated: true
                )
            }
        }))
...
final class HomeCoordinator {
    ...
    func presentScanActions(for item: ScanService.ScanItem, sourceView: UIView?, from presenter: UIViewController) {
        ...
        alertVC.addAction(UIAlertAction(title: L("common.details"), style: .default, handler: { [weak self, weak presenter] _ in
            ...
            self.scanDetailsWorkflow.presentDetails(for: item, from: presenter)
        }))
        ...
        alertVC.addAction(UIAlertAction(title: L("common.rename"), style: .default, handler: { [weak self, weak presenter] _ in
            guard let presenter else { return }
            self?.scanLibraryEditWorkflow.presentRename(
                for: item,
                from: presenter,
                onScansChanged: { [weak self] in self?.onScansChanged?() },
                makeAlert: { [weak self] title, message in
        ...
        alertVC.addAction(UIAlertAction(title: L("common.delete"), style: .destructive, handler: { [weak self, weak presenter] _ in
            guard let presenter else { return }
            self?.scanLibraryEditWorkflow.presentDelete(
                for: item,
                from: presenter,
                onScansChanged: { [weak self] in self?.onScansChanged?() },
                makeAlert: { [weak self] title, message in
