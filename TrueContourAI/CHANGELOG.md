...
- Test-architecture cleanup: connected-device UI test target build succeeded, and focused `AppRuntimeSettingsTests`, `PreviewStoreTests`, `PreviewCoordinatorExportTests`, and `ScanStoreTests` passed on `Riy's iPhone` on 2026-03-20; at that point one representative save/reopen device-smoke rerun was still failing, which led directly to the later save/reopen stabilization fix below
- Save/reopen stabilization: connected-device build succeeded, focused `PreviewCoordinatorTests`, `PreviewCoordinatorExportTests`, `PreviewStoreTests`, and `ScanStoreTests` passed on `Riy's iPhone`, and `TrueContourAIDeviceSmokeTests.testDeviceSmokeSaveThenReopenFromHome` passed again on 2026-03-20 after retaining the post-scan preview button action target for the lifetime of the presented preview
